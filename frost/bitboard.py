...
# Bit manipulation
def flipBit(bboard: int, pos: int) -> int: return bboard ^ (1 << pos) # Flip bit at pos.
def setBit(bboard: int, pos: int) -> int: return bboard | (1 << pos) # Set bit at pos to be 1.
def clearBit(bboard: int, pos: int) -> int: return bboard & ~(1 << pos) # Set bit at pos to be 0.
def getBit(bboard: int, pos: int) -> int: return 1 if (bboard & (1 << pos)) != 0 else 0


def ls1b(bboard: int) -> int: return bboard & -bboard 
def ms1b(bboard: int) -> int:
...
def fBitscan(bboard: int) -> int:
    ...
    index: List[int] = [0, 1, 2, 8, 3, 15, 9, 22, 4, 29,
    ...
    debrujin: int = 0x1061438916347932A5CD9D3EAD7B77F 
    shift: int = ls1b(bboard)
    return index[((debrujin * shift) & 0xFE000000000000000000000000000000) >> 121]



def rBitscan(bboard: int) -> int:
    """
    ...
    index: List[int] = [0, 69, 1, 27, 70, 113, 2, 13, 28, 97,
    ...
    debrujin: int = 0x1FC47709ECA6B19CC17D25B45754379
